document.addEventListener("DOMContentLoaded", function () {
  ...
  const translations = {
    ar: {
      ...
      early_desc: "نفتح الآن المجال لأول العملاء الذين يرغبون في بناء أنظمة أتمتة وذكاء اصطناعي مع GENEX.",
    ...
    en: {
      ...
      service_card_1_desc: "Autonomous units enhanced with an autonomous artificial intelligence agent that interact with customers and execute real tasks.",
      ...
      early_desc: "We are now opening access to our first partners who want to build AI and automation systems with GENEX.",
  ...
  if (canvas) {
    ...
    function createStrands() {
      ...
      while (strands.length < strandCount && attempts < strandCount * 50) {
        ...
        if (!overlaps) strands.push(candidate);
      }
    }
    ...
    function drawStrand(points) {
      ...
      for (let i = 0; i < pA.length; i++) {
        ...
        ctx.lineTo(pB[i].x, pB[i].y);
        ctx.strokeStyle = i % 2 === 0 ? "rgba(255,255,255,0.24)" : "rgba(177,18,38,0.24)";
        ctx.lineWidth = 1;
        ctx.stroke();
      }
